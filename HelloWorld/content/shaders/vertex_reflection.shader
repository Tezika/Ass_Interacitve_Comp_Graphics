...
void main()
{
	gl_Position = mat_projection * mat_view * mat_model * vec4(i_pos, 1);
	vec4 normalInterp4 = mat_normalToView * vec4(i_normal, 1);
	normalInterp = vec3(normalInterp4) / normalInterp4.w;
	vec4 vertexPos4 = mat_view * mat_model * vec4(i_pos, 1);
